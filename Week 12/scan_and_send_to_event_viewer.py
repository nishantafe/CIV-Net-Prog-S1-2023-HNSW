...
def scan(an_ip, a_port):
    ...
    if result == 0:
        ...
        status_message = f"{an_ip}:{a_port:<5d} Closed/Filtered or host is offline | Time: {timestamp}"
        print("\033[31m", status_message)
        status_messages.append(status_message)
    sock.close()
    log_file_out.close()
...
ips = [subnet_prefix + "." + str(num) for num in range(11, 50, 2)]
...
write_event_viewer_log(status_messages, 0)
